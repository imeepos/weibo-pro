...
```tsx
import { Button, Card, CardHeader, CardTitle, CardContent } from '@sker/ui/components/ui'
...
```tsx
import { ChartContainer, ChartTooltip, ChartTooltipContent } from '@sker/ui/components/ui'
...
```tsx
import { Editor } from '@sker/ui/components/ui/editor'
...
```tsx
import { useDebounce, useBoolean, useMount } from '@sker/ui/hooks/*'
...
1. **基础组件**：放在 `components/ui/`，遵循 Radix UI + CVA 模式
2. **复合组件**：放在 `components/blocks/`，组合多个基础组件
3. **业务组件**：根据领域放在对应目录（workflow/editor/weibo）

### 样式约定

- 使用 TailwindCSS 原子类，避免内联样式
- 颜色使用 CSS 变量（`text-foreground`, `bg-primary`）
- 使用 `cn()` 合并类名
- 响应式使用 `sm:`, `md:`, `lg:` 前缀
- 深色模式使用 `dark:` 前缀
...
import { WorkflowCanvas } from '@sker/ui/components/workflow/workflow-canvas'
```

### 快捷导出
```typescript
import { Button, Card, Badge } from '@sker/ui/components/ui'  // 批量导出
import { WorkflowCanvas, WorkflowNode } from '@sker/ui/components/workflow'
```
...
### 图表渲染优化
- ECharts 使用懒加载（按需引入图表类型）
- 大数据集使用 `dataZoom` 和 `sampling`
- 3D 图使用 Web Worker
...
### 工作流画布
- React Flow 使用 `nodesDraggable={false}` 提升性能
- 大量节点时启用 `onlyRenderVisibleElements`
- 使用 `useCallback` 包裹事件处理器

## 常见问题

### Q: 深色模式如何切换？
A: 使用 `next-themes` 的 `ThemeProvider`：
```tsx
...
```css
:root {
  --primary: oklch(0.5 0.2 250);  /* 自定义主色调 */
}
```

### Q: 工作流节点如何自定义渲染？
A: 实现自定义节点组件并注册到 `nodeTypes`：
```tsx
const nodeTypes = {
  myNode: MyCustomNode,
}
...
```tsx
import { EditorKit } from '@sker/ui/components/editor/editor-kit'
const customPlugins = EditorKit.filter(p => p.key !== 'ai')
```
...
- **存在即合理**：每个组件必须有不可替代的理由存在
- **优雅即简约**：代码自文档化，拒绝无意义注释
- **性能即艺术**：优化不应牺牲代码美感
- **组件复用**：优先组合而非继承
- **类型安全**：所有组件导出完整 TypeScript 类型

每个组件都是艺术品，每次提交都是文化遗产。
