...
import type { DouyinClient } from './douyin-client'
...
export class DouyinLogin implements ILogin {
  ...
  async loginByQrcode(): Promise<LoginResult> {
    ...
    try {
      ...
      await page.waitForURL('https://www.douyin.com/', { timeout: 120000 })
  ...
  async isLoggedIn(): Promise<boolean> {
    try {
      ...
      return cookies.some((c: any) => c.name === 'sessionid' || c.name === 'sid_guard')
    } catch {
      return false
    }
  }
