...
// 2. 获取服务实例
const userService = root.get(UserService);
```
...
// 子注入器可以访问父注入器的服务
const config = appInjector.get(Config); // 从 root 获取
```
...
root.set([
  {
    provide: APP_INITIALIZER,
    useFactory: (db: Database, cache: Cache) => async () => {
      await Promise.all([
        db.connect(),
        cache.warmup()
      ]);
...
@Module({
  providers: [
    // 从 @sker/core 根注入器获取服务
    {
      provide: UserService,
      useFactory: () => root.get(UserService)
    }
  ]
})
...
1. **依赖注入（Dependency Injection）** - 核心设计模式
2. **单例模式（Singleton）** - 根注入器全局唯一
3. **工厂模式（Factory）** - 提供多种 Provider 工厂
4. **装饰器模式（Decorator）** - 使用 TypeScript 装饰器
5. **策略模式（Strategy）** - 多种注入策略（Optional, Self 等）
6. **模板方法模式（Template Method）** - 生命周期钩子

## 性能优化

1. **实例缓存**：同一作用域内单例
2. **位标志优化**：使用位运算加速标志检查
3. **懒加载**：支持 LazyClassProvider 和 LazyFactoryProvider
4. **依赖路径追踪**：仅在检测到循环依赖时构建完整路径

## 注意事项

1. **循环依赖**：尽量避免，必要时使用 `forwardRef()`
2. **作用域选择**：根据服务生命周期选择合适的 `providedIn`
3. **多值注入**：使用 `multi: true` 收集同一 Token 的多个实例
4. **资源清理**：实现 `OnDestroy` 接口释放资源
5. **类型安全**：使用 `InjectionToken<T>` 确保类型安全

## 最佳实践

1. **优先使用 `@Injectable({ providedIn: 'root' })`**：自动注册，无需手动配置
2. **使用 InjectionToken 替代字符串令牌**：类型安全
3. **合理使用参数装饰器**：`@Optional()`, `@Self()` 等
4. **实现生命周期钩子**：`@OnInit()`, `OnDestroy`
5. **使用 APP_INITIALIZER 处理异步初始化**：确保应用启动顺序
