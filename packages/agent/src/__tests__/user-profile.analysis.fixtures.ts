...
export function originalStdDev(values: number[]): number {
...
export function originalSimpleSimilarity(str1: string, str2: string): number {
...
export function originalTextSimilarity(texts: string[]): number {
...
export function originalExtractSource(sourceHtml: string): string {
  const match = sourceHtml.match(/>([^<]+)</);
  return match ? match[1]! : 'unknown';
}

export function originalTimeRegularity(
  hourDist: number[],
  stdDev: number,
  avgInterval: number
): number {
...
export function originalContentMechanical(
  similarity: number,
  avgLength: number
): number {
...
export function originalAnalyzeTimeBehavior(posts: any[]) {
...
export function originalAnalyzeContentFeatures(posts: any[]) {
...
export function originalAnalyzeInteractionFeatures(posts: any[]) {
...
export function originalAnalyzeDeviceSources(posts: any[]) {
...
export function originalDetectSignals(posts: any[], nlpResults: any[]) {
...
export function makePost(overrides: Partial<Record<string, unknown>> = {}) {
  return {
...
export const samplePosts = Array.from({ length: 60 }, (_, i) =>
  makePost({
    created_at: new Date(
