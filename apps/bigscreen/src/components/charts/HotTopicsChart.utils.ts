...
// 获取趋势对应的颜色（十六进制）
export function getTrendColorHex(trend: "up" | "down" | "stable"): string {
  switch (trend) {
    case "up":
      return "#10b981";
    case "down":
...
export interface BuildHotTopicsChartOptionParams {
  data: HotTopic[];
  title: string;
  maxTopics: number;
...
export function buildHotTopicsChartOption({
  data,
  title,
  maxTopics,
  orientation,
}: BuildHotTopicsChartOptionParams) {
