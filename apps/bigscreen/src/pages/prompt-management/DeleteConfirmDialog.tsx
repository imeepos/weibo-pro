  ...
  return (
    <Dialog open={!!target} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        ...
        <DialogFooter>
          <button onClick={onClose} className="rounded-md bg-muted px-3 py-1.5 text-sm">取消</button>
          <button onClick={onConfirm} className="rounded-md bg-destructive px-3 py-1.5 text-sm text-white">删除</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
