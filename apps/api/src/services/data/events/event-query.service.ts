...
import type { TimeRange, HotEvent, EventListItem, EventWithCategory, EventStatistics, EventCategoryStats, InfluenceUser, GeographicResponse, EventSentimentHotness, EventSentimentDistribution, EventSentimentIntensity, EventKeywordTimeSeries, EventKeywordBySentiment, EventNegativeKeywordAlert, EventEventTypeDistribution, EventEngagementTrend, EventAnomaly, EventPeak, UserRelationNetwork } from './types';
...
export class EventQueryService {
  ...
  async getEventList(
    ...
    return await this.cacheService.getOrSet(
      ...
      async () => {
        ...
        // 【性能优化】当事件数量超过阈值时，回退到数据库 hotness 字段排序
        // 回退模式不应用实时时间衰减，hotness 值可能不够"新鲜"，建议后台任务定期更新。
        if (needsPaging) {
          getStructuredLogger().warn('Event query fallback to database sorting due to large event count', { type: 'performance_fallback', eventCount: total, threshold: MAX_HOTNESS_CALCULATION_EVENTS, timeRange: statsTimeRange, page, pageSize });

          const events = await findEventList(statsTimeRange, { limit: pageSize, offset: (page - 1) * pageSize, search, category });
          ...
          const data = events.map((event) => {
            ...
            return mapEventToListItem(event, stats ? [stats] : [], displayHotness);
          });
        ...
        const displayHotnessMap = await calculateDecayedHotnessForEvents(allEventIds, statsTimeRange, lambda);
        ...
        getStructuredLogger().info('Event hotness sorting result', { type: 'hotness_sort_debug', eventCount: sortedEventIds.length, eventsWithStats: displayHotnessMap.size, top10: sortedEventIds.slice(0, 10).map(id => ({ eventId: id.substring(0, 8), hotness: Math.round((displayHotnessMap.get(id) ?? 0) * 100) / 100 })) });
        ...
        setImmediate(async () => {
          try {
            ...
            getStructuredLogger().info('Successfully persisted event hotness to database', { type: 'hotness_persisted', eventCount: displayHotnessMap.size, timeRange: statsTimeRange, lambda });
          } catch (error) {
            getStructuredLogger().error('Failed to persist event hotness to database', { type: 'hotness_persist_error', eventCount: displayHotnessMap.size, error: error instanceof Error ? error.message : String(error) });
          }
        });
      ...
      async () => {
        return await useEntityManager(async (entityManager) => {
          ...
          const totalStats = await entityManager
            ...
            .addSelect(`COUNT(DISTINCT COALESCE(NULLIF(post.region_name, ''), NULLIF(user.location, ''), '未知'))`, 'totalregioncount')
            .addSelect('AVG((nlp.sentiment->>\'positive_prob\')::numeric - (nlp.sentiment->>\'negative_prob\')::numeric)', 'globalavgsentiment')
          ...
          const normalizedGlobalSentiment = globalAvgSentiment !== 0 ? Math.round(Math.max(0, Math.min(1, (globalAvgSentiment + 1) / 2)) * 100) / 100 : 0.5;
          ...
          const locationData = await entityManager
            ...
            .select(`COALESCE(NULLIF(post.region_name, ''), NULLIF(user.location, ''), '未知')`, 'location')
            .addSelect('COUNT(DISTINCT user.id)', 'usercount')
            .addSelect('COUNT(post.id)', 'postcount')
            .addSelect('AVG((nlp.sentiment->>\'positive_prob\')::numeric - (nlp.sentiment->>\'negative_prob\')::numeric)', 'avgsentiment')
          ...
          const result = locationData.map((item: {
            ...
            const normalizedSentiment = avgSentiment !== 0 ? Math.max(0, Math.min(1, (avgSentiment + 1) / 2)) : 0.5;
          ...
          return {
            statistics: { regionCount: realTotalRegions, userCount: realTotalUsers, postCount: realTotalPosts, avgSentiment: normalizedGlobalSentiment },
  ...
  async getHotEvents(timeRange: TimeRange): Promise<HotEvent[]> { return this.basicQueries.getHotEvents(timeRange); }
  async getEventById(id: string): Promise<EventWithCategory | null> { return this.basicQueries.getEventById(id); }
  async getEventCategories(timeRange: TimeRange): Promise<EventCategoryStats> { return this.basicQueries.getEventCategories(timeRange); }
  async getLatestStatistics(eventId: string): Promise<EventStatistics | null> { return getLatestStatistics(eventId); }
  async getEventStatistics(eventId: string, timeRange: TimeRange): Promise<EventStatistics[]> { return getEventStatistics(eventId, timeRange); }
  async getAllEventStatistics(eventId: string): Promise<EventStatistics[]> { return getAllEventStatistics(eventId); }
  async getInfluenceUsers(eventId: string): Promise<InfluenceUser[]> { return this.userQueries.getInfluenceUsers(eventId); }
  async getEventKeywords(eventId: string, limit: number = 1000): Promise<Array<{ keyword: string; weight: number; sentiment: string }>> { return this.keywordQueries.getEventKeywords(eventId, limit); }
  async getSentimentHotness(eventId: string): Promise<EventSentimentHotness[]> { return this.sentimentQueries.getSentimentHotness(eventId); }
  async getSentimentDistribution(eventId: string): Promise<EventSentimentDistribution> { return this.sentimentQueries.getSentimentDistribution(eventId); }
  async getSentimentIntensity(eventId: string): Promise<EventSentimentIntensity[]> { return this.sentimentQueries.getSentimentIntensity(eventId); }
  async getKeywordsTimeSeries(eventId: string, topN: number = 20): Promise<EventKeywordTimeSeries[]> { return this.keywordQueries.getKeywordsTimeSeries(eventId, topN); }
  async getKeywordsBySentiment(eventId: string): Promise<EventKeywordBySentiment[]> { return this.keywordQueries.getKeywordsBySentiment(eventId); }
  async getNegativeKeywords(eventId: string, threshold: number = 0.5): Promise<EventNegativeKeywordAlert[]> { return this.keywordQueries.getNegativeKeywords(eventId, threshold); }
  async getEventTypes(eventId: string): Promise<EventEventTypeDistribution[]> { return this.sentimentQueries.getEventTypes(eventId); }
  async getEngagementTrend(eventId: string): Promise<EventEngagementTrend[]> { return this.engagementQueries.getEngagementTrend(eventId); }
  async getAnomalies(eventId: string): Promise<EventAnomaly[]> { return this.engagementQueries.getAnomalies(eventId); }
  async getPeaks(eventId: string, limit: number = 168): Promise<EventPeak[]> { return this.engagementQueries.getPeaks(eventId, limit); }
  async getEventUserRelations(eventId: string): Promise<UserRelationNetwork> { return this.userQueries.getEventUserRelations(eventId); }
}
