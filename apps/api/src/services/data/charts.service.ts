...
export class ChartsService {
  ...
  private async fetchSentimentTrend(timeRange: TimeRange): Promise<ChartData> {
    return useEntityManager(async (manager) => {
      ...
      try {
        chartData = await this.fetchSentimentTrendFromStatistics(manager, start, end, granularity);
      } catch (error) {
        this.logger?.warn('Statistics table query failed, fallback to NLP results', error);
        chartData = await this.fetchSentimentTrendFromNLPResults(manager, start, end, granularity);
      }
  ...
  private async fetchSentimentTrendFromStatistics(manager: any, start: Date, end: Date, granularity: string): Promise<ChartData> {
  ...
  private async fetchSentimentTrendFromNLPResults(manager: any, start: Date, end: Date, granularity: string): Promise<ChartData> {
